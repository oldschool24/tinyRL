from Brain.model import PolicyModel, QuantPolicyModel, PredictorModel, TargetModel, HalfPolicyModel, NonTraceable, FxQuantPolicyModel
import torch
...
class Brain:
    ...
    def train(self, states, actions, int_rewards,
              ...
              log_probs, next_int_values, next_ext_values, total_next_obs, is_binary_qat=False):
        if is_binary_qat:
            self.binary_qat()
    ...
    def set_from_checkpoint(self, checkpoint, quantization):
        """Download trained model and quantize policy if necessary.

        :param checkpoint: trained model
        :param quantization: type of quantization (dynamic_int8, dynamic_float16, static_int8, static_float16, binary)
        """
        self.current_policy.load_state_dict(checkpoint["current_policy_state_dict"])
        if 'dynamic' in quantization:    # post training dynamic quantization
            dtype = torch.qint8 if 'int8' in quantization else torch.float16
            self.current_policy = torch.quantization.quantize_dynamic(
                self.current_policy,     # the original model
                {torch.nn.Linear},       # a set of layers to dynamically quantize
                dtype=dtype              # the target dtype for quantized weights
            )
            print(dtype)
        elif 'static' in quantization:   # post training static quantization
            if 'int8' in quantization:
                ...
                self.current_policy.qconfig = qconfig
        elif 'binary' in quantization:
            bconfig = BConfig(
                activation_pre_process=torch.nn.Identity,
                activation_post_process=torch.nn.Identity,
    ...
    def print_time_of_policy(self, number=500):
        """Print the time that is spent on a forward pass of policy model.
    ...
    def binary_qat(self):
        ...
        prepare_binary_model(self.current_policy, bconfig,
                             ignore_layers_name=['conv1', 'conv2', 'conv3', 'fc2', 'extra_value_fc',
                                                 'extra_policy_fc', 'policy', 'int_value', 'ext_value'])
